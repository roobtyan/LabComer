...
void CommandUpdateTable1::on_pushButton_clicked()
{
    // 获取开机指令相关参数
    ...
    // 校验通过，发送到计算命令
    QString result = updateTable1Values(Table1Num, CurrentLimit, VoltageLimit, ControlledCurrent, P1, P2, PressureSetting, MainDCDC, CathodeSelector,
                  FeedBranch, GasSelector, TimeDuration, usedTable2Num, usedTable3Num);
    // 发送到槽函数
    emit sendUpdateTable1CommandToMainWindow(result);
    this -> close();
}

QString CommandUpdateTable1::updateTable1Values(QString Table1Num, QString CurrentLimit, QString VoltageLimit, QString ControlledCurrent,
...
//    unsigned char head5 = 0x02; // 指令值，根据不同指令要更换
    ...
    // 计算2字节校验码
    unsigned short CRC = head1 + head2 + head3 + head4 + 0x10;
    for (int i = 0; i < 22; i++)
    {
        CRC += lookupTable1[i];
    }
    commandResultGen(&resultTemp, "EB90001B21E2250010", 22, lookupTable1, CRC);
